...
function _sheetResumen(wb, accounts, transactions) {
  ...
  const assets = _sumCategory(accounts, 'Activo Circulante', 'Activo No Circulante', 'Activo Diferido');
  const liabilities = _sumCategory(accounts, 'Pasivo Circulante', 'Pasivo No Circulante', 'Pasivo Diferido');
  const equity = _sumCategory(accounts, 'Capital Social', 'Utilidades');
  const income = _sumCategory(accounts, 'Ingresos Operacionales', 'Ingresos No Operacionales');
  const expenses = _sumCategory(accounts, 'Gastos Operacionales', 'Gastos No Operacionales');
...
function _sheetCuentas(wb, accounts) {
  ...
  const categorias = [
    'Activo Circulante', 'Activo No Circulante', 'Activo Diferido',
    'Pasivo Circulante', 'Pasivo No Circulante', 'Pasivo Diferido',
...
function _sheetEstadosFinancieros(wb, accounts, transactions) {
  ...
  // Detalle de cuentas por categoría (una o varias)
  const byCategory = (...cats) => accounts.filter(a => cats.includes(a.category));
  ...
  const rows = [
    ...
    ...byCategory('Ingresos Operacionales').map(a => [`  ${a.name}`, Number(a.balance)]),
    ['INGRESOS NO OPERACIONALES', ''],
    ...byCategory('Ingresos No Operacionales').map(a => [`  ${a.name}`, Number(a.balance)]),
    ['Total Ingresos', income],
    [],
    ['GASTOS OPERACIONALES', ''],
    ...byCategory('Gastos Operacionales').map(a => [`  ${a.name}`, Number(a.balance)]),
    ['GASTOS NO OPERACIONALES', ''],
    ...byCategory('Gastos No Operacionales').map(a => [`  ${a.name}`, Number(a.balance)]),
    ...
    [],
    ['ACTIVOS CIRCULANTES', ''],
    ...byCategory('Activo Circulante').map(a => [`  ${a.name}`, Number(a.balance)]),
    ['Subtotal Activos Circulantes', _sumCategory(accounts, 'Activo Circulante')],
    [],
    ['ACTIVOS NO CIRCULANTES', ''],
    ...byCategory('Activo No Circulante').map(a => [`  ${a.name}`, Number(a.balance)]),
    ['Subtotal Activos No Circulantes', _sumCategory(accounts, 'Activo No Circulante')],
    [],
    ['ACTIVOS DIFERIDOS', ''],
    ...
    [],
    ['PASIVOS CIRCULANTES', ''],
    ...byCategory('Pasivo Circulante').map(a => [`  ${a.name}`, Number(a.balance)]),
    ['Subtotal Pasivos Circulantes', _sumCategory(accounts, 'Pasivo Circulante')],
    [],
    ['PASIVOS NO CIRCULANTES', ''],
    ...byCategory('Pasivo No Circulante').map(a => [`  ${a.name}`, Number(a.balance)]),
    ['Subtotal Pasivos No Circulantes', _sumCategory(accounts, 'Pasivo No Circulante')],
    [],
    ['Total Pasivos', liabilities],
    [],
    ['CAPITAL SOCIAL', ''],
    ...byCategory('Capital Social').map(a => [`  ${a.name}`, Number(a.balance)]),
    ['UTILIDADES', ''],
    ...byCategory('Utilidades').map(a => [`  ${a.name}`, Number(a.balance)]),
  ...
  const secciones = [
    'INGRESOS OPERACIONALES','INGRESOS NO OPERACIONALES',
    'GASTOS OPERACIONALES','GASTOS NO OPERACIONALES',
    'ACTIVOS CIRCULANTES','ACTIVOS NO CIRCULANTES','ACTIVOS DIFERIDOS',
    'PASIVOS CIRCULANTES','PASIVOS NO CIRCULANTES',
    'CAPITAL SOCIAL','UTILIDADES'
  ];
  ...
  const totales = [
    ...
    'Subtotal Activos Circulantes','Subtotal Activos No Circulantes','Subtotal Activos Diferidos','Total Activos',
    'Subtotal Pasivos Circulantes','Subtotal Pasivos No Circulantes','Total Pasivos',
    'Total Patrimonio',
...
/** Suma saldos de todas las cuentas de una o varias categorías */
function _sumCategory(accounts, ...categories) {
  return accounts
