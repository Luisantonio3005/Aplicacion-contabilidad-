...
function renderTAccounts(accounts, transactions) {
  ...
  const hasMultiCurrency = new Set(transactions.map(t => t.currency)).size > 1;
  const currencyNote = hasMultiCurrency ? ' *' : '';
  setText('totalEntrada', fmtCurrency(totalEntrada, 'MXN') + currencyNote);
  setText('totalSalida',  fmtCurrency(totalSalida,  'MXN') + currencyNote);
}
...
const CURRENCY_SYMBOLS = { MXN: '$', USD: 'US$', EUR: '€', COP: 'Col$', ARS: 'AR$' };
