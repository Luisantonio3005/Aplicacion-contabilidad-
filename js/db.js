...
function addAccount(name, category, type, balance, costType, currency) {
  ...
  try {
    db.run('BEGIN');
    db.run(
      `INSERT INTO accounts (name, category, type, balance, costType, currency, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, category || 'Activo', type, Number(balance) || 0, costType || null, currency || 'MXN', now, now]
    );
